...
class ExportCoordinatesDialog(QDialog):
    ...
    def _build_cartool_els_groups(self):
        """
        Build the electrode groups used by Cartool ELS.
        ...
        Coordinates are continuous voxel coordinates in the native T1 image.
        """
        electrodes = getattr(self.state, "electrodes", []) or []
        ...
        for elec_idx in selected_indices:
            if not (0 <= int(elec_idx) < len(electrodes)):
                continue

            electrode = electrodes[int(elec_idx)]

            electrode_name = str(electrode.get("name", f"Electrode_{int(elec_idx) + 1}")).strip()

            if not electrode_name:
                electrode_name = f"Electrode_{int(elec_idx) + 1}"
    ...
    def _write_cartool_els(self, path: Path) -> None:
        """
        Write a Cartool ELS file matching the supplied reference format.
        ...
        for group in groups:
            ...
            # Cartool electrode type used in the reference ELS file.
            lines.append("1")
        ...
        # Windows line endings, as in the supplied Cartool file.
        path.write_text(
            "\r\n".join(lines) + "\r\n",
            encoding="utf-8",
        )
    ...
    def _on_export(self):
        ...
        try:
            ...
            # ---------------------------------------------------------
            # 2) Cartool ELS export
            #
            # This block must remain OUTSIDE the coordinate-system loop.
            # ELS uses its own Cartool coordinate conversion and does not
            # require LPS / RAS / VOX to be selected.
            # ---------------------------------------------------------
            if "els" in formats:
